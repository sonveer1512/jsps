...
class Customlib {
    ...
    function getDatepickerFormat($date_only = true, $time = false) {
        ...
        $admin = $this->CI->session->userdata('pmsadmin');
        if ($admin) {
            if ($date_only && !$time) {
    ...
    function getStaffID() { // users table id of users
        $session_Array = $this->CI->session->userdata('pmsadmin');
        $staff_id = $session_Array['id'];
        return $staff_id;
    }

    function getSessionLanguage() {
        $student_session = $this->CI->session->userdata('pmsadmin');
	...
	public function setLogReport($username, $role,$logmessage) {
        ...
        $this->CI->custom_model->addlogreport($data);
    }
    ...
    public function getClientId() {
        $result = $this->CI->custom_model->getClientId();

        if(!empty($result)) {
            $sub = str_replace("CI","",$result[0]['client_unique_id']);
            $sub = $sub + 1;
            $client_id = "CI".str_pad($sub, 5, '0', STR_PAD_LEFT);
        }else{
            $client_id = "CI".str_pad("1", 5, '0', STR_PAD_LEFT);
        }
    ...
    public function sendmail($sender_email, $reciever_email, $subject, $message) {
        $this->CI->load->library('email');
        ...
        $config['smtp_user']    = 'suryapratap05021995@gmail.com';
        $config['smtp_pass']    = 'Surya@1995';
        ...
        $this->email->from('suryapratap05021995@gmail.com', 'Surya Pratap');
